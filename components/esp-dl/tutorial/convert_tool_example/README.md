# Customize a Model Step by Step [[中文]](./README_cn.md)

This tutorial shows how to customize a model with our [convert tool](../../tools/convert_tool/README.md). In this tutorial, the example is an runnable project about [MNIST](https://tensorflow.google.cn/datasets/catalog/mnist?hl=en) classification mission, hereinafter referred to as MNIST.

Note:
- If your model has already been quantized on other platforms (e.g. TFLite int8 model), and the quantization scheme is different from that of ESP-DL (see [Quantization Specification](../../docs/en/quantization_specification.md)), then the model cannot be deployed with ESP-DL.
- If your model has not been quantized yet, you can deploy it with reference to [quantization_tool_example](../quantization_tool_example/README.md).
